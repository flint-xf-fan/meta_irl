...
class MetaIRLNPO(MetaIRLBatchPolopt):
    ...
    def optimize_policy(self, itr, samples_data, expert_traj_batch):
        ...
        input_list += obs_list + action_list + adv_list + [
            np.tile(np.expand_dims(expert_traj_batch, axis=1), [
                1, samples_data[0]['observations'].shape[0] //
                self.irl_model.T, 1, 1
            ])
        ]
        state_info_list, dist_info_list = {
            k: []
        ...
        state_info_list = {
            k: np.concatenate(state_info_list[k])
            for k in self.policy.state_info_keys
        }
        dist_info_list = {
            k: np.concatenate(dist_info_list[k])
            for k in self.policy.distribution.dist_info_keys
        }
        input_list += tuple(state_info_list.values()) + tuple(
            dist_info_list.values())
        ...
        self._wandb_dict['LossBefore'] = loss_before
        self._wandb_dict['LossAfter'] = loss_after
        self._wandb_dict['MeanKLBefore'] = mean_kl_before
        self._wandb_dict['MeanKL'] = mean_kl
        self._wandb_dict['LossDiff'] = loss_before - loss_after
